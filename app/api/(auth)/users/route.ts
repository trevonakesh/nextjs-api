...
export const PATCH = async (request: Request) => {
    try {
        ...
        if (!userId || !newUsername) {
            return new NextResponse(
                JSON.stringify({ massege: "ID or New username not found" }),
                {
                    status: 400,
                }
            );
        ...
        if (!updatedUser) {
            return new NextResponse(
                JSON.stringify({ massege: "User not found in the databese" }),
                { status: 400 }
            );

        }
        return new NextResponse(
            JSON.stringify({ massege: "User is updated", user: updatedUser }),
            { status: 200 }
        )
...
export const DELETE = async (request: Request) => {
    try {
        ...
        if (!userId) {
            return new NextResponse(
                JSON.stringify({ massege: "ID not found" }),
                { status: 400 }
            );
        }

        if (!Types.ObjectId.isValid(userId)) {
            return new NextResponse(JSON.stringify({ masssege: "Invalid User Id" }),
                { status: 400 });
        ...
        if (!deleteUser) {
            return new NextResponse(JSON.stringify({ massage: "User not found in the database" }),
                { status: 400 }
            );
        }
        
        return new NextResponse(JSON.stringify({ massage: "User is deleted" }),
            { status: 200 }
        );
